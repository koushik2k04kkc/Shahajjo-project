...
export default function ContinuityChart({ data, loading = false }) {
  ...
  return <div className="transition-opacity duration-200">
    ...
    <div className="overflow-x-auto scrollbar-none"><svg viewBox={`0 0 ${width} ${height}`} className="min-w-[620px] w-full" role="img" aria-label={t.chartAria}>
      ...
      <text x={bandX+bandWidth/2} y={top+14} textAnchor="middle" fontSize="10" fontWeight="600" fill="#9A6700">{language === 'BN' ? data.shortageWindow.labelBn || t.chartShortage : data.shortageWindow.label}</text>
      {data.points.map((point,i) => <text key={point.time} x={x(i)} y={height-14} textAnchor="middle" fontSize="10" fill="#94A3B8">{formatClock(point.time, language)}</text>)}
    </svg></div>
  </div>
}
