...
export default function UnifiedApp() {
  ...
  const login = () => {
    ...
    if (role === 'agent') localStorage.setItem('shahajjo_user', JSON.stringify({ id: 'AGT-2048', name: 'Mahmud Rahman', outlet: 'Rahman Telecom', phone: '01700-000000' }))
    setSignedIn(true)
  }
  const switchRole = () => { localStorage.removeItem('shahajjo_session'); setSignedIn(false); setRole(null); localStorage.removeItem('shahajjo_role') }
  ...
  if (!signedIn) return <Gateway language={language} onLanguage={toggleLanguage}><form onSubmit={(event) => { event.preventDefault(); login() }} className="w-full max-w-sm rounded-3xl border border-slate-200 bg-white p-7 shadow-card"><button type="button" onClick={clearRole} className="mb-6 text-xs font-bold text-slate-500">← Change role</button><Brand/><h1 className="mt-6 text-2xl font-bold">{roles.find((item) => item.id === role)?.label} sign in</h1><p className="mt-1 text-sm text-slate-500">Prototype credentials are accepted.</p><label className="mt-6 block text-xs font-bold text-slate-600">Mobile or email<input className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3" defaultValue={role === 'agent' ? '01700-000000' : 'demo@shahajjo.bd'}/></label><label className="mt-4 block text-xs font-bold text-slate-600">PIN<input type="password" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3" defaultValue="1234"/></label><button className="mt-6 w-full rounded-xl bg-brand py-3 font-bold text-white">Continue to workspace</button></form></Gateway>
